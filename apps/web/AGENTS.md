...
- **Database:** Always connect to the database through a data.ts file with `import server-only` at the top
  - the data/$model.ts file imports from @repo/db directly
  - a `use server` actions.ts file imports from data/$model.ts file
